...
{
    $_REQUEST = array_merge(['preset_id' => 0, 'fields'    => [],], $_REQUEST);
    ...
    if ($mode === 'import')
    {
        ...
        if (file_exists($archives_path) && $uploaded_files = fn_get_dir_contents($archives_path, false, true))
        {
        ...
        if (!empty($preset))
        {
            ...
            try
            {
                ...
                {
                    $fields_mapping = array_combine(array_column($_REQUEST['fields'], 'name'), $_REQUEST['fields']);
                }
                else
                {
                ...
                $remapping_schema = $presets_importer->getEximSchema($schema,$fields_mapping, $pattern);
                if ($remapping_schema)
                {
                    $presets_importer->setPattern($pattern);
                    $result = $reader->getContents(null, $schema);
                    $result->showNotifications();
                    $import_items = $presets_importer->prepareImportItems($result->getData(), $fields_mapping, $preset['object_type'], true, $remapping_schema);
                    $presets_manager->updateState(['preset_id'      => $preset['preset_id'], 'last_launch'    => TIME, 'last_status'    => ImportStatuses::IN_PROGRESS, 'file'           => $preset['file'], 'file_type'      => $preset['file_type'],]);
                    ...
                    $is_success = fn_execute_as_company(static function () use ($pattern, $import_items, $preset)
                    {
            ...
            else
            {
                ...
                {
                    if (!fn_advanced_import_check_file_is_archive($file) && !in_array($file, ['.', '..']))
                    {
    ...
    if ($mode === 'export_example')
    {
        ...
        foreach ($preset['fields'] as $field_data)
        {
            ...
            $data[0][$field] = isset($relations[$field_type]['fields'][$field]['example_value']) ? $relations[$field_type]['fields'][$field]['example_value'] : '';
            $pattern_data = isset($pattern['export_fields'][$field]) ? $pattern['export_fields'][$field] : [];
