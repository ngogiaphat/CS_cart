...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    if ($mode === 'upload')
    {
        ...
        {
            fn_set_notification(NotificationSeverity::ERROR, __('error'), __('text_not_allowed_to_upload_file_extension', ['[ext]' => fn_get_file_ext($file['name'])]));
            exit;
        }
        ...
        {
            $redirect_url = Url::buildUrn(['import_presets', 'manage'], ['object_type' => $old_preset['object_type'],]);
        }
    ...
    if ($mode === 'update')
    {
        $redirect_url = !empty($_REQUEST['preset_id']) ? 'import_presets.update?preset_id=' . $_REQUEST['preset_id'] : 'import_presets.add?object_type=' . $_REQUEST['object_type'];
        if (
            empty($_REQUEST['file']) && (
        ...
        $preset = array_merge(['preset_id'      => 0, 'file_type'      => PresetFileTypes::LOCAL, 'file'           => '',], $_REQUEST);
        $file = $file_manager->uploadPresetFile($preset);
        if ($file)
        {
            $file = reset($file);
        ...
        if (isset($preset['uploaded_archives']))
        {
            ...
            if (file_exists($temp_images_path) && $preset['archive_path'] === $temp_images_path)
            {
    ...
    if ($mode === 'm_delete')
    {
        $_REQUEST = array_merge(['preset_ids'   => [], 'object_type'  => 'products', 'redirect_url' => 'import_presets.manage',], $_REQUEST);
    ...
    if ($mode === 'delete')
    {
        $_REQUEST = array_merge(['preset_id' => 0, 'object_type' => 'products',], $_REQUEST);
    ...
    {
        $params = array_merge(['modifier' => '', 'value'    => '', 'notify'   => YesNo::YES,], $_REQUEST);
...
if ($mode === 'update' || $mode === 'add' || $mode === 'get_fields')
{
...
if ($mode === 'manage')
{
    $params = array_merge(['page'              => 1, 'object_type'       => 'products', 'items_per_page'    => Registry::get('settings.Appearance.admin_elements_per_page')], $_REQUEST);
    if (fn_allowed_for('MULTIVENDOR'))
    {
        list($common_presets, $search) = fn_get_import_presets(array_merge($params, ['company_id'     => 0, 'items_per_page' => 0 ])); // Gets all common presets
        ...
        if ($common_presets)
        {
            list($modifiers_presense,) = $presets_manager->find(
                false,
                [
                    ['modifier', '<>', ''], 'ipf.preset_id' => array_keys($common_presets),
                ],
                [
                    [
                        'table' => ['?:import_preset_fields' => 'ipf'], 'condition' => ['ip.preset_id = ipf.preset_id'],
                    ],
                    [
                        'table' => ['?:import_preset_descriptions' => 'ipd'], 'condition' => ['ip.preset_id = ipd.preset_id'],
                    ],
                ],
    ...
    if (empty($common_presets) && empty($presets) && $params['page'] === 1)
    {
    ...
    if ($presets)
    {
        list($modifiers_presense,) = $presets_manager->find(
            false,
            [
                ['modifier', '<>', ''], 'ipf.preset_id' => array_keys($presets),
            ],
            [
                [
                    'table'     => ['?:import_preset_fields' => 'ipf'], 'condition' => ['ip.preset_id = ipf.preset_id'],
...
if ($mode === 'add')
{
    $preset = array_merge(['object_type' => 'products',], $_REQUEST);
    ...
    Registry::set('navigation.tabs', [
        ...
        'fields'  => ['title'        => __('advanced_import.fields_mapping'), 'href'         => 'import_presets.get_fields', 'ajax'         => true, 'ajax_onclick' => true,],
        'options' => ['title' => __('settings'), 'js'    => true,],
    ]);
    Tygh::$app['view']->assign(['pattern'     => $pattern, 'preset'      => $preset, 'is_mve'      => fn_allowed_for('MULTIVENDOR'), 'allowed_ext' => fn_advanced_import_get_allowed_extensions(),]);
}
if ($mode === 'update')
{
    ...
    list($presets) = fn_get_import_presets(['preset_id' => $_REQUEST['preset_id'],]);
    if (!$presets)
    {
        return [CONTROLLER_STATUS_NO_PAGE];
    }
    ...
    {
        $path = rtrim($preset['options']['images_path']['input_prefix'] . $preset['options']['images_path']['display_value'], '/') . '/' . $file_manager::TEMP_ARCHIVE_FOLDER . $preset['preset_id'];
...
if ($mode === 'get_fields')
{
    ...
    $preset = array_merge(['file'           => '', 'file_type'      => PresetFileTypes::LOCAL, 'preset_id'      => 0, 'object_type'    => 'products', 'fields'         => [], 'options'        => [], 'company_id'     => null,], $_REQUEST);
    ...
    elseif ($preset['file']) {
        ...
        try
        {
            ...
            if ($result->getData())
            {
                $preview = $presets_importer->prepareImportItems($result->getData(), $preset['fields'], $preset['object_type']);
            }
            $pattern = $presets_manager->getPattern($preset['object_type']);
            $preset = $presets_manager->mergePattern($preset, $pattern);
            Tygh::$app['view']->assign([
...
{
    if (!isset($_REQUEST['path']) || !isset($_REQUEST['company_id']) || !isset($_REQUEST['option_id']))
    {
        return [CONTROLLER_STATUS_NO_PAGE];
    }
    ...
    return [CONTROLLER_STATUS_REDIRECT, sprintf('file_editor.manage?in_popup=1&path=%s&container_id=%s', $path, md5(TIME))];
}
if ($mode === 'get_archive')
{
    list($presets) = fn_get_import_presets(['preset_id' => $_REQUEST['preset_id'],]);
