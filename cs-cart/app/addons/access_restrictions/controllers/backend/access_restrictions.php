...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    if ($mode === 'm_update_statuses' && !empty($_REQUEST['item_ids']) && !empty($_REQUEST['status']))
    {
    ...
    if ($rule_data['section'] === 'ip' || $rule_data['section'] === 'admin_panel') {
        ...
        if (!empty($rule_data['range_from']) || !empty($rule_data['range_to']))
        {
            ...
            if (fn_validate_ip($range_from, true) && fn_validate_ip($range_to, true))
            {
                ...
                if ($rule_data['section'] === 'admin_panel' && Registry::get('addons.access_restrictions.admin_reverse_ip_access') !== YesNo::YES && ($self_block_host || $self_block_proxy))
                {
